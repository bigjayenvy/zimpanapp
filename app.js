...
const LEGAL = {
  privacy: {
    ...
    body: [
      ...
      ['Cookies', 'One essential cookie holds a random session token so you stay signed in. The analytics named below set their own cookies to count visits and see which pages get used — that is measurement, not advertising, and we do not run ad pixels or sell audiences. If you would rather not be counted, any browser-level tracking blocker will stop all of it, as will Google\'s own opt-out for its part, and the app works exactly the same either way.'],
      ['Analytics', 'We use Google Analytics, Google Tag Manager and Metricool to understand how the site is used in aggregate — visits, devices, which parts people reach. All of it runs on the public pages and never has sight of your entries, notes or amounts, which are only ever fetched after you sign in. Google and Metricool each process what they collect on their own terms, as separate controllers.'],
